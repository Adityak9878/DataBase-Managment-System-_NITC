...
int BlockAccess::renameRelation(char oldName[ATTR_SIZE], char newName[ATTR_SIZE])
{
    ...
    RecId search_index=linearSearch(RELCAT_RELID,RELCAT_ATTR_RELNAME,newRelationName,EQ);

    // If relation with name newName already exists (result of linearSearch
    //                                               is not {-1, -1})
    //    return E_RELEXIST;
    if(search_index.block!=-1 && search_index.slot!=-1)
    return E_RELEXIST;
    ...
    // search the relation catalog for an entry with "RelName" = oldRelationName
     search_index=linearSearch(RELCAT_RELID,RELCAT_ATTR_RELNAME,oldRelationName,EQ);
    ...
    */
    ...
    relcatBuffer.getRecord(relAttribute,search_index.slot);
    /* update the relation name attribute in the record with newName.
       (use RELCAT_REL_NAME_INDEX) */
    strcpy(relAttribute[RELCAT_REL_NAME_INDEX].sVal,newName);   
    // set back the record value using RecBuffer.setRecord
    relcatBuffer.setRecord(relAttribute,search_index.slot);
    /*
    ...
    */
    ...
    for(int i=0;i<relAttribute[RELCAT_NO_ATTRIBUTES_INDEX].nVal;i++)
    {
        RecId id=linearSearch(ATTRCAT_RELID,ATTRCAT_RELNAME,oldRelationName,EQ);
        RecBuffer attrCatBlock(search_index.block);
        Attribute attrCatRecord[ATTRCAT_NO_ATTRS];
        attrCatBlock.getRecord(attrCatRecord,id.block);
        strcpy(attrCatRecord[ATTRCAT_REL_NAME_INDEX].sVal,newName);
        attrCatBlock.setRecord(attrCatRecord,id.slot);
    }
...
int BlockAccess::renameAttribute(char relName[ATTR_SIZE], char oldName[ATTR_SIZE], char newName[ATTR_SIZE]) {
    ...
    // Search for the relation with name relName in relation catalog using linearSearch()
     RecId search_index=linearSearch(RELCAT_RELID,RELCAT_ATTR_RELNAME,relNameAttr,EQ);
    ...
    while (true) {
        ...
        search_index=linearSearch(ATTRCAT_RELID,ATTRCAT_ATTR_RELNAME,relNameAttr,EQ);
        ...
        RecBuffer attrCatBlock(search_index.block);
        Attribute attrCatRecord[ATTRCAT_NO_ATTRS];  
        attrCatBlock.getRecord(attrCatRecord, search_index.slot);
        // if attrCatEntryRecord.attrName = oldName
        //     attrToRenameRecId = block and slot of this record
        if(strcmp(attrCatRecord[ATTRCAT_ATTR_NAME_INDEX].sVal,oldName)==0)
        {
            attrToRenameRecId.block = search_index.block;
            attrToRenameRecId.slot=search_index.slot;
			break;
        }
    ...
    if(attrToRenameRecId.block==-1 && attrToRenameRecId.block==-1)
